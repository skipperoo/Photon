...
QImage GpuDenoiser::denoise(
    ...
    // Placeholder: Return null QImage for now until full Vulkan BM3D pipeline is ported
    // This allows Denoiser::denoise to fallback to Hybrid path (GPU search + CPU filter)
    return QImage();
}
