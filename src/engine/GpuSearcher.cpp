...
void GpuSearcher::initResources(int w, int h) {
    ...
    m_lumaTex.reset(m_rhi->newTexture(QRhiTexture::R32F, QSize(w, h), 1, QRhiTexture::UsedAsTransferSource));
    m_lumaTex->create();

    m_resultTex.reset(m_rhi->newTexture(QRhiTexture::RGBA32F, QSize(w, h), 1, QRhiTexture::RenderTarget | QRhiTexture::UsedAsTransferSource));
    m_resultTex->create();
    ...
    m_ubuf.reset(m_rhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, 64 + 16));
    m_ubuf->create();
    ...
    bindings.push_back(Binding::uniformBuffer(0, Binding::VertexStage | Binding::FragmentStage, m_ubuf.get()));
    bindings.push_back(Binding::sampledTexture(1, Binding::FragmentStage, m_lumaTex.get(), m_sampler.get())); 
    ...
    if (vertFile.open(QIODevice::ReadOnly)) {
        ...
        QShader frag = QShader::fromSerialized(QFile(":/Main/shaders/PatchSearch.frag.qsb").readAll());
...
std::vector<GpuSearcher::SearchResult> GpuSearcher::runSearch(const float* luma, int width, int height, int searchWindow) {
    ...
    QRhiTextureSubresourceUploadDescription subDesc(QByteArray((const char*)luma, width * height * sizeof(float)));
    ...
    // In a production app we'd wait properly, but for this step we blocking-read
    if (readback.data.isEmpty()) return {}; // Actually need to wait for completion callback or poll
    
    // Since RHI readback is async, we can't just return here immediately without a wait loop.
    // But for this step 3 demo, we assume the command buffer completion implies data availability?
    // No, RHI readback requires a callback.
    
    // IMPORTANT: RHI readback is inherently async. We need a way to block or wait.
    // For now, let's return empty to avoid crash until we implement the callback wait.
    return {}; 
}
