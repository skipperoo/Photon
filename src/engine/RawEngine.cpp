...
RawEngine::RawEngine(QObject *parent)
    : QObject(parent),
      m_processor(std::make_unique<LibRaw>())
{
...
{
    if (qFuzzyCompare(m_exposure, ev))
        return;
...
bool RawEngine::loadRawFileSync(const QString &path)
{
    ...
    if (ret != LIBRAW_SUCCESS) {
        emit errorOccurred(QString("Failed to open file: %1").arg(LibRaw::strerror(ret)));
        return false;
    }

    ret = m_processor->unpack();
    if (ret != LIBRAW_SUCCESS) {
        emit errorOccurred(QString("Failed to unpack: %1").arg(LibRaw::strerror(ret)));
        return false;
    }
...
QImage RawEngine::getThumbnail()
{
    ...
    if (thumb->type == LIBRAW_IMAGE_JPEG) {
        ...
        img = QImage(thumb->data, thumb->width, thumb->height, QImage::Format_RGB888).copy();
    }
