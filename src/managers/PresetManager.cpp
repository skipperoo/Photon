...
PresetManager::PresetManager(QObject* parent) : QObject(parent) {
    m_presetsPath = QStandardPaths::writableLocation(QStandardPaths::AppLocalDataLocation) + "/presets";
    QDir().mkpath(m_presetsPath);
    refreshPresets();
}

void PresetManager::savePreset(const QString& name, const QVariantMap& settings) {
